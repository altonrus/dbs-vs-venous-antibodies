...
p01<-ggplot(tb3, aes(samplemonth, n, colour = factor(SER_ADM_COV))) + 
  geom_point(aes(colour = factor(SER_ADM_COV)), size = 3) + 
  geom_line(aes(colour = factor(SER_ADM_COV)), size=1) + 
  labs(title = "Sampling Frequency",
       x="Date of Sample Collection", y="Num. of Samples",
       color = "") +
  scale_color_manual(labels = c("DBS", "Venous"), values=c("blue", "red")) +
  theme_bw() 

# Print all plots panelled
png(file="../3_intermediate/Fig.S5 Heatmap and Freq.png", 
