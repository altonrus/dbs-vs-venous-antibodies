...
The purpose of those code is to figure of where the problem is that causing the server lag problems. The problem emerged when cleaning the APL data.

```{r}
# Load a random data for DBS vs Venous project:
load("~/dbs-vs-venous-antibodies/1_data/private/CLSA_Alberta_PSM.RData")
View(CLSA_mAB)

# run a simple model building using the data
lm10.t<-glm(SER_SPIKE_COV~SER_ADM_COV, 
          data = CLSA_mAB)
gtsummary::tbl_regression(lm10.t, exponentiate = TRUE)

rm(lm10.t)

# Load the APL data when the problem happens:
load("~/COVID-assay-adjust-methods/1_data/private/RFD4682e.RData")
load("~/COVID-assay-adjust-methods/1_data/private/RFD4682e1.RData")

rm(RFD4682_e)
rm(RFD4682_e1)
```
## 5. Making plots for the 
