...
#### 2.10 Modified Fig S5 Scatterplot, the solid lines indicate rolling mean (3-mon average)

##### Calculate 3-month rolling average

```{r}
# Function to calculate the 3-month rolling average for a given month M
rolling_n <- function(M, data) {
  start_date <- M - 45  # First day of previous month
  end_date <-   M + 44  # Last day of next month
  avg_data <- data %>%
    filter(start_datetime_COV >= start_date & start_datetime_COV <= end_date) %>%
    filter(SER_NUCLEOCAPSID_COV==1) %>%
    group_by(SER_ADM_COV, PROV_5) %>%
    summarise(rolling_avg = mean(NUCLEOCAPSID_ANTIBODY, na.rm = TRUE), .groups = "drop") %>%
    mutate(start_datetime_COV = M)
  return(avg_data)
}

# Calculate the rolling average for all dates
rolling_n_avg <- lapply(CLSA_com50$start_datetime_COV, rolling_n, data = CLSA_com50) %>%
  bind_rows()

# modify the rolling avg data for ggplot
rolling_n_avg<-rolling_n_avg %>%
  mutate(SER_ADM_COV = factor(SER_ADM_COV, levels = c(0, 1), labels = c("Venous", "DBS")))

# Function to calculate the 3-month rolling average for a given month M
rolling_s <- function(M, data) {
  start_date <- M - 45  # First day of previous month
  end_date <-   M + 44  # Last day of next month
  avg_data <- data %>%
    filter(start_datetime_COV >= start_date & start_datetime_COV <= end_date) %>%
    filter(SER_SPIKE_COV==1) %>%
    group_by(SER_ADM_COV, PROV_5) %>%
    summarise(rolling_avg = mean(SPIKE_ANTIBODY, na.rm = TRUE), .groups = "drop") %>%
    mutate(start_datetime_COV = M)
  return(avg_data)
}

# Calculate the rolling average for all dates
rolling_s_avg <- lapply(CLSA_com50$start_datetime_COV, rolling_s, data = CLSA_com50) %>%
  bind_rows()

# modify the rolling avg data for ggplot
rolling_s_avg<-rolling_s_avg %>%
  mutate(SER_ADM_COV = factor(SER_ADM_COV, levels = c(0, 1), labels = c("Venous", "DBS")))
```

```{r}
# add rolling S/Co into the figure
p2.10.a<-CLSA_com50 %>%
  mutate(SER_NUCLEOCAPSID_COV = factor(SER_NUCLEOCAPSID_COV, 
                                       levels = c(0, 1), 
                                       labels = c("Negative", "Positive")),
  ...
  ggplot(aes(x = start_datetime_COV)) +
  geom_point(aes(x = start_datetime_COV, y = NUCLEOCAPSID_ANTIBODY, 
                 color = SER_ADM_COV, shape = SER_NUCLEOCAPSID_COV), alpha=0.6) +
  geom_line(data=rolling_n_avg,
            aes(x = start_datetime_COV, y = rolling_avg, color = SER_ADM_COV, 
                group = SER_ADM_COV), 
            linewidth = 0.75)+
  scale_color_manual("", values = c("DBS" = "blue", "Venous" = "red")) +
  scale_shape_manual("", values = c("Negative" = 1, "Positive" = 16)) +
...
p2.10.b<-CLSA_com50 %>%
  ...
  ggplot(aes(x = start_datetime_COV)) +
  geom_point(aes(x = start_datetime_COV, y = SPIKE_ANTIBODY, 
                 color = SER_ADM_COV, shape = SER_SPIKE_COV), alpha=0.6) +
  geom_line(data=rolling_s_avg,
            aes(x = start_datetime_COV, y = rolling_avg, color = SER_ADM_COV, 
                group = SER_ADM_COV), 
            linewidth = 0.75)+
  scale_color_manual("", values = c("DBS" = "blue", "Venous" = "red")) +
...
png(file="../4_output/figs/Fig.S5 Scatterplot.png", 
    width=2400, height=2400, res=300)
cowplot::plot_grid(p2.10.a, p2.10.b,
                   labels = c("A", "B"),
                   ncol = 1, nrow = 2)
dev.off()
```

#### 2.11 Supplemental Table X
