...
## create urban/rural indicator - need to move it to the right place
...
```{r}
# Add model 06.t specific variable into the data

CLSA_com50$dist_min_25km<-CLSA_com50$dist_min/25000        # traveling distance - per 25km
CLSA_com50$age10<-CLSA_com50$AGE_NMBR_COVID/10             # age - per 10 yr
...
tbl_regression(lm06.t, exponentiate = T)
```
...
# We decided to present the ORs using model 06.t (w 3-cat time var) and CLSA_com50 data (N=7230), allow us to utlize more data.
...
# The "Outbreak Peak" is replaced with 3-cate time variable
```
