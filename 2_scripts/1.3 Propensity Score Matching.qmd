...
- use different methods: we used subclass instead of nearest. This matching methods matched within subclasses created based on propensity score. Usually they use 6 subclasses but in our case since it's difficult to reach balance, we will use 8.
...
simple_reg_OR(CLSA_com50$URBAN_RURAL_COVID)                   # Rural/Urban
...
g01<-ggplot(OR_import, aes(x = OR, y = group)) + 
    ...
    geom_point(size = 2, aes(colour=Adj),
               position = position_dodge(width = 1.0))+
    theme_bw()+
