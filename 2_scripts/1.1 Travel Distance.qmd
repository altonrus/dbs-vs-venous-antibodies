...
Notes: Statistic Canada changes the address of those shape files from time to time. If the address above does not work, search for "Provinces/Territories, Cartographic Boundary File" and look for 2016 Census data.
...
coords_temp<-st_coordinates(centroid$coord)

# Transform the coordinates that's readable to GMap
FSA_Center_Converted<-centroid %>%
  mutate(
    coord_text = paste0(coords_temp[, 2], ", ", coords_temp[, 1])
  )
rm(coords_temp)
...
## Save a copy
save(FSA_Center_small, file = "~/dbs-vs-venous-antibodies/1_data/AB_Geo/FSA_Center_fixed.RData")
```
...
coords_temp<-st_coordinates(centroid_10$coord)

# Transform the coordinates that's readable to GMap
centroid_10_cov<-centroid_10 %>%
  mutate(
    coord_text = paste0(coords_temp[, 2], ", ", coords_temp[, 1])
  )
rm(coords_temp)
