---
title: "1.0 PCM_Data Linking & Cleaning"
...
### 1.1 Check the CLSA Covid Datasets
...
#### Create new var representing general health
...
# 1.8 Heart, lung, kidney, liver, or pancreas failure
...
# 1.10 Pneumonia  confirmed using chest X-rays
...
# 2.3 add a col for all health conditions
```
...
2. PCR and other available tests:
```{r}
...
CLSA_Time<-CLSA_Antibody %>%
  select(entity_id, start_datetime_COV, SER_HOSP_COV: SER_TEST20_COV, # Test dates
...
# All those with negative and multiple vac date have two doses
...
CLSA_Temp<-merge(CLSA_com50, select(CLSA_Time, entity_id, time_vac, time_vac15), by='entity_id',
      all.x = T, all.y = F)

table(CLSA_Temp$time_vac15, CLSA_Temp$SER_ADM_COV)
```

```{r}
proptable_chisq2<-function(r){tab1<-table(r, CLSA_Temp$SER_ADM_COV)
...
proptable_chisq2(CLSA_Temp$time_vac15)
```


### Updates Aug 21st 2023:

### 1.4 CLSA quantitative assay results are availble, merge it
...
#### 2. Alberta Subset (A good example for traveling dist analysis)
